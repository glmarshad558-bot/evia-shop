...
class User(db.Model, UserMixin):
    ...
    username = db.Column(db.String(150), unique=True, nullable=False) # Increased size
    password = db.Column(db.String(500), nullable=False) # FIX: Increased to 500 to stop crash
...
class Product(db.Model):
    ...
    image_2 = db.Column(db.String(500)) # NEW: For second photo
    description = db.Column(db.Text)    # NEW: For detail page
...
def admin():
    ...
    if request.method == 'POST':
        p = Product(
            ...
            image_2=request.form.get('image_url_2')      # NEW
        )
        db.session.add(p)
        db.session.commit()
...
# NEW: Route for the Product Detail Page (Amazon/Flipkart Style)
...
def signup():
    if request.method == 'POST':
        ...
        # FIX: The hash generated by scrypt is long, so User.password must be String(500)
